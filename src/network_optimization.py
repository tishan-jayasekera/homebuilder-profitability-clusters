...
def calculate_shortfalls(events_df: pd.DataFrame, targets_df: pd.DataFrame = None, period_days: int = None, total_events_df: pd.DataFrame = None) -> pd.DataFrame:
    ...
    if not events_df.empty:
        ...
        period_actuals = pd.DataFrame(columns=period_keys + ['Period_Referrals'])
    
    # --- 2. Cumulative Actuals (For Progress) ---
    if not cumulative_source.empty:
        ...
        cum_actuals = pd.DataFrame(columns=group_keys + ['Actual_Referrals'])
    ...
    if targets_df is None:
        ...
        if 'LeadTarget_from_job' in cols_available and 'WIP_JOB_LIVE_END' in cols_available:
            ...
            builders = cumulative_source['Dest_BuilderRegionKey'].dropna().unique() if not cumulative_source.empty else []
...
def analyze_network_leverage(events_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if events_df.empty:
        ...
        cols = ['MediaPayer_BuilderRegionKey', 'Dest_BuilderRegionKey', 'Suburb', 'Transfer_Rate', 'eCPR', 'CPR_base', 'Total_Referrals_Sent']
        return pd.DataFrame(columns=[c for c in cols if c != 'Suburb'] + (['Suburb'] if 'Suburb' in events_df.columns else []))
...
def generate_global_media_plan(shortfall_df: pd.DataFrame, leverage_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Check if leverage data is available (prevent crash on empty)
    has_leverage = not leverage_df.empty and 'Dest_BuilderRegionKey' in leverage_df.columns
...
def generate_investment_strategies(focus_builder: str, shortfall_data: pd.DataFrame, leverage_data: pd.DataFrame, events_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Check if leverage data is usable
    if leverage_data.empty or 'Dest_BuilderRegionKey' not in leverage_data.columns:
        return pd.DataFrame()
