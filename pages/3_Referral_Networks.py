...
def render_interactive_network(G, selected_cluster_id):
    """Render PyVis/AGraph interactive network."""
    nodes = []
    edges = []
    
    # Graph visual properties
    for node_id in G.nodes():
        # Only highlight nodes in current cluster or neighbors
        node_cluster = G.nodes[node_id].get('cluster', 0)
        
        # Simple Logic: If cluster selected, only show that cluster
        if node_cluster == selected_cluster_id:
            nodes.append(Node(
                id=node_id, 
                label=str(node_id), 
                size=25, 
                color="#1D4ED8",
                title=f"Builder: {node_id}\nCluster: {node_cluster}"
            ))
    ...
    if not nodes:
        st.info("No nodes in this cluster to visualize.")
        return
...
def main():
    ...
    with tab_explorer:
        ...
        if G.nodes:
            ...
            # Render AGraph
            render_interactive_network(G, sel_cluster_id)
        else:
